...
export async function GET(req: NextRequest) {
  ...
  let { data: project, error: projectError } = await supabaseAdmin
    .from('projects')
    .select('id, name, slug, description, category, avatar_url, website_url, whitepaper_url, github_url, twitter_url, discord_url, trust_score, is_verified, has_token, show_holders, show_votes, token_address')
  ...
  if (projectError) {
    ...
    if (fallback) {
      project = { ...fallback, has_token: false, show_holders: true, show_votes: true, whitepaper_url: null, github_url: null, twitter_url: null, discord_url: null, token_address: null }
    }
  }
