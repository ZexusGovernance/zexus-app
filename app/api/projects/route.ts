...
export async function GET() {
  const { data, error } = await supabaseAdmin
    .from('projects')
    .select('id, name, slug, category, description, trust_score, is_verified, avatar_url, has_token, admin_wallet, created_at')
    .order('trust_score', { ascending: false })

  if (error) return NextResponse.json({ error: error.message }, { status: 500 })

  // Expose has_admin flag (whether real person manages this project) but not the wallet itself
  const projects = (data ?? []).map(({ admin_wallet, ...p }) => ({
    ...p,
    has_admin: !!admin_wallet,
  }))
...
export async function PATCH(req: NextRequest) {
  ...
  const { wallet, name, description, category, website_url, whitepaper_url, github_url, twitter_url, discord_url, avatar_url, show_holders, show_votes, has_token, token_address } = body
  ...
  if (typeof token_address === 'string')                updates.token_address  = token_address.trim() || null
  ...
  const { data: updated, error } = await supabaseAdmin
    ...
    .update(updates)
    .eq('id', project.id)
    .select('id, slug, name, description, category, avatar_url, website_url, whitepaper_url, github_url, twitter_url, discord_url, trust_score, is_verified, has_token, show_holders, show_votes, token_address')
    .single()
