...
export default function PostCard({ post, onClick, onCommentClick, index = 0, onVoteFinalized }: PostCardProps) {
  const isEmergency  = post.isEmergency || (post.type === 'alert')
  const isDbPost     = isUUID(post.id)
  const isRealVoting = isDbPost && post.type === 'voting'
  ...
  const castVote = async (e: React.MouseEvent, vote: 'confirm' | 'dispute') => {
    ...
    try {
      ...
      setVoteData(prev => prev ? { ...prev, confirmCount: d.confirmCount, disputeCount: d.disputeCount, total: d.total, userVote: d.userVote } : prev)
    } catch { setVoteHint('Vote failed'); setTimeout(() => setVoteHint(null), 2500) }
    finally { setVoteCasting(false) }
  }

  const isVotingOpen = post.type === 'voting' && (voteData ? voteData.isOpen : post.vote?.open)
      ...
      <div className="card-body">
        ...
        {isRealVoting && voteData && (
          <div style={{ marginTop: 10 }}>
            {/* Weight bar — based on power, not raw count */}
            {voteData.total > 0 && (() => {
              const totalW  = (voteData.confirmWeight ?? voteData.confirmCount) + (voteData.disputeWeight ?? voteData.disputeCount)
              const yesPct  = totalW > 0 ? Math.round(((voteData.confirmWeight ?? voteData.confirmCount) / totalW) * 100) : 50
              return (
            ...
            <div style={{ marginTop: 7 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: 'var(--muted2)', marginBottom: 4 }}>
                <span style={{ display: 'flex', alignItems: 'center', gap: 5 }}>
                  {voteData.total}/{voteData.votes_needed} wallets
                  {voteData.userWeight != null && (
                    <span style={{ color: 'var(--muted)', fontStyle: 'normal' }}>· your weight: <strong style={{ color: 'var(--text)' }}>{voteData.userWeight}</strong></span>
                  )}
                </span>
                <span style={{ color: voteData.isOpen ? 'var(--gold)' : voteData.passed ? 'var(--green)' : 'var(--muted)' }}>
                  {voteData.isOpen ? voteData.timeLeft : voteData.passed ? '✓ Passed' : '✗ Not passed'}
                </span>
              </div>
              <div style={{ height: 3, background: 'rgba(255,255,255,0.07)', borderRadius: 2, overflow: 'hidden' }}>
                <div style={{
                  height: '100%', borderRadius: 2,
                  width: `${Math.min(100, (voteData.total / voteData.votes_needed) * 100)}%`,
                  background: voteData.total >= voteData.votes_needed ? 'var(--green)' : 'var(--gold)',
        ...
        {/* Demo vote data fallback */}
        {!isRealVoting && post.vote && (
          ...
          <div style={{ display: 'flex', gap: 5, alignItems: 'center', position: 'relative' }}>
            {voteHint && (
              <div style={{
                ...
                padding: '4px 9px', fontSize: 11, color: 'var(--muted)',
            ...
            <button
              style={{
                ...
                cursor: voteCasting ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
                display: 'flex', alignItems: 'center', gap: 4, transition: 'all 0.15s',
                background: voteData?.userVote === 'confirm' ? 'rgba(83,201,146,0.12)' : 'rgba(255,255,255,0.05)',
                color: voteData?.userVote === 'confirm' ? 'var(--green)' : 'var(--muted)',
                opacity: voteData?.userVote && voteData.userVote !== 'confirm' ? 0.38 : 1,
                fontWeight: voteData?.userVote === 'confirm' ? 600 : 400,
              }}
              onClick={e => castVote(e, 'confirm')}
            ...
            <button
              style={{
                ...
                cursor: voteCasting ? 'not-allowed' : 'pointer', fontFamily: 'inherit',
                display: 'flex', alignItems: 'center', gap: 4, transition: 'all 0.15s',
                background: voteData?.userVote === 'dispute' ? 'rgba(224,112,112,0.1)' : 'rgba(255,255,255,0.05)',
                color: voteData?.userVote === 'dispute' ? 'var(--red)' : 'var(--muted)',
                opacity: voteData?.userVote && voteData.userVote !== 'dispute' ? 0.38 : 1,
                fontWeight: voteData?.userVote === 'dispute' ? 600 : 400,
              }}
              onClick={e => castVote(e, 'dispute')}
              disabled={voteCasting}
            >
