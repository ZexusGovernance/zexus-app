...
const TYPES = [
  ...
  { type: 'voting'  as PostType, label: 'Voting',  icon: 'ti-chart-bar',      color: '#c9a55a', desc: 'Open community vote · 48h · trust score changes if confirmed' },
      ...
      <div className="post-modal create-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: 540 }}>
        ...
        <div className="post-modal-scroll" style={{ padding: '0 0 4px' }}>
          ...
          <div style={{ padding: '16px 20px 0' }}>
            ...
            <div style={{ fontSize: 11, color: 'var(--muted)', marginTop: 8, paddingBottom: type === 'voting' ? 10 : 14, borderBottom: type === 'voting' ? 'none' : '0.5px solid var(--border)' }}>
              <i className={`ti ${meta.icon}`} style={{ color: meta.color, marginRight: 5 }} />
              {meta.desc}
            </div>
            {type === 'voting' && (
              <div style={{ paddingBottom: 14, borderBottom: '0.5px solid var(--border)',
                display: 'flex', alignItems: 'center', gap: 8 }}>
                <i className="ph-bold ph-info" style={{ color: 'var(--muted2)', fontSize: 13, flexShrink: 0 }} />
                <span style={{ fontSize: 11, color: 'var(--muted)' }}>
                  If vote passes → Trust Score <span style={{ color: 'var(--green)', fontWeight: 600 }}>+5 pts</span> · fixed by platform
                </span>
              </div>
            )}
          </div>
